...
int main(void)
{
    ...
    real rmoon = 1737.0;               // Moon radius [km]
    real rearth = 6378.0;              // Earth radis [km]
    real r12 = 384400.0;               // Earth-Moon distance [km]
    real m1 = 5974.e21;                // Earth mass [kg]
    real m2 = 7348.e19;                // Moon mass [kg]
    real M = m1 + m2;                  // Total mass [kg]
    real pi_1 = m1 / M;                // Earth mass ratio [-]
    real pi_2 = m2 / M;                // Moon mass ratio [-]
    real mu1 = 398600;                 // G *m1;                             // Earth grav. parameter [km^3 /s^2]
    real mu2 = 4903.02;                // G * m2;                 // Moon grav. parameter [km^3 /s^2]
    real mu = mu1 + mu2;               // Earth-Moon system grav. parameter [km^3 /s^2]
    real W = sqrt(mu / pow(r12, 3.0)); // Angular velocity Moon around Earth [rad/s]
    ...
    real phi = _dynorb_deg2rad(-90.0);                                   // Polar Azimuth coordinate of s/c (+ counterclockwise from Earth-Monn line) [rad]
    real v0 = 10.9148;                                                   // initial speed of spacecraft relative to rotating Earth-Moon system [km/s]
    real gamma = _dynorb_deg2rad(20.0);                                  // Initial flight path angle [rad]
    ...
    /* ----------------------------------------------------------
     * STEP 4: Open files and postprocess and log data:
     * ---------------------------------------------------------- */
    ...
    for (int i = 0; i < solverConf.n_steps; i++)
    {
        ...
        fprintf(outfile, "%.20f %.20f %.20f\n", threeBodyRestrictSys.tt[i],
                threeBodyRestrictSys.YY_t[i * threeBodyRestrictSys.sys_size] * 1.e-5,
                threeBodyRestrictSys.YY_t[i * threeBodyRestrictSys.sys_size + 1] * 1.e-5);
    }

    // Final State:
    real xf = threeBodyRestrictSys.YY_t[(solverConf.n_steps - 1) * threeBodyRestrictSys.sys_size];
    real yf = threeBodyRestrictSys.YY_t[(solverConf.n_steps - 1) * threeBodyRestrictSys.sys_size + 1];
    real vxf = threeBodyRestrictSys.YY_t[(solverConf.n_steps - 1) * threeBodyRestrictSys.sys_size + 2];
    real vyf = threeBodyRestrictSys.YY_t[(solverConf.n_steps - 1) * threeBodyRestrictSys.sys_size + 3];

    // Post-processing:
    real df = sqrt(pow(xf - x2, 2.0) + pow(yf, 2.0)) - rmoon; // Final distance from the Moon [km]
    real vf = sqrt(pow(vxf, 2.0) + pow(vyf, 2.0));            // Final relative speed [km/s]

    // Print Info:
    printf("\n\n\n");
    printf("------------------------------------------------------------\n");
    printf("Example 2.18: Lunar trajectory using the restricted 3-body eq.s.\n");
    printf("Initial Earth altitude [km] = %g\n", d0);
    printf("Initial angle between radial and Earth-Moon line [deg] = %g\n", _dynorb_rad2deg(phi));
    printf("Initial flight path angle [deg] = %g\n", _dynorb_rad2deg(gamma));
    printf("Flight time [days] = %g\n", tf / sec_in_day);
    printf("Final distance from the moon [km] = %g\n", df);
    printf("Final relative speed [km/s] = %g\n", vf);
    printf("------------------------------------------------------------\n");
    ...
    const char *plot_command1 =
        ...
        "set title 'Example 18 Chapter 02: Mass under GravityRestricted 3-body problem - Position [1.e5 km]'\n"
        "set xlabel 'x(t) [1.e5 km]'\n"
        "set ylabel 'y(t) [1.e5 km]'\n"
        "set grid\n"
