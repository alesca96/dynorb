...
int main(void)
{
    ...
    FILE *outfile = fopen("./data/ex_01_19-b.txt", "w");
    if (outfile == NULL)
    {
    ...
    const char *plot_command =
        ...
        "plot './data/ex_01_19-b.txt' using 1:2 with lines lc rgb 'red' title 'x(t)', "
        "'./data/ex_01_19-b.txt' using 1:4 with lines lc rgb 'black' title 'x_{a}(t)'\n";

    // const char *plot_command =
    //     "set terminal qt enhanced\n"
    //     "set title 'Example 19 Chapter 01: Simple Harmonic Oscillator using Custom RKF45'\n"
    //     "set xlabel 'Time t [s]'\n"
    //     "set ylabel 'x(t) [m]'\n"
    //     "plot './data/ex_01_19-b.txt' using 1:2 with lines lc rgb 'red' title 'x(t)'\n";

    // "'./data/ex_01_19-b.txt' using 1:3 with points pt 7 ps 0.5 lc rgb 'blue' title 'v(t)', "
